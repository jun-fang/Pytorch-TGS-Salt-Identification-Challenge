...
# import sys
# sys.path.append('/home/jun/tgs/code/common')
# from include import *
...
## https://github.com/moskomule/pytorch.snapshot.ensembles
class SnapshotScheduler():
    def __init__(self, initial_lr, epochs_per_cycle, min_lr=0):
        super(SnapshotScheduler, self).__init__()
        self.initial_lr  = initial_lr
        self.epochs_per_cycle = epochs_per_cycle
        self.min_lr = min_lr

    def cosine_annealing_lr(initial_lr, iteration, epoch_per_cycle, min_lr=0):
        return min_lr + (initial_lr - min_lr) * (cos(pi * iteration / epoch_per_cycle) + 1) / 2

    def get_rate(self, epoch):
        j = epoch % self.epochs_per_cycle
        lr = cosine_annealing_lr(self.initial_lr, j, self.epochs_per_cycle, min_lr=self.min_lr)
        return lr

    def __str__(self):
        string = 'Snapshot Learning Rates\n' \
                + 'epochs_per_cycle=%d, initial_lr=%0.5f, min_lr=%0.5f'%(self.epochs_per_cycle, self.initial_lr, self.min_lr)
        return string
...
if __name__ == '__main__':
    ...
    for i in range(4):
        for j in range(epochs_per_cycle):
            lr = cosine_annealing_lr(initial_lr, j, epochs_per_cycle, min_lr=min_lr)
            lrs[epoch] = lr
            if lr<0:
                num_epoches = epoch
                break
    ...
    fig = plt.figure()
    plot_rates(fig, lrs, num_epoches, title=str('csss'))
    plt.show()
